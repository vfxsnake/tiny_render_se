...
**Step 6b is open — the Phong reflection model — and the session that opened it went into *deriving* the reflection vector rather than typing it.** `PhongShader` is a copy of `LambertShader` carrying the same varyings (`std::array<Vec3f, 3> varyingNormals_`, unchanged — 6b moves the **model** axis only, never the rate) plus three uniforms: `viewDirection_`, `ambient_`, `shininess_`. The constructor normalizes both `lightDirection_` and `viewDirection_`; `vertex()` is Lambert's verbatim; `fragment()` still stops at diffuse. The model chosen is the classic mirror reflection `R = 2(N·L)N - L` with `(R·V)^shininess`, **not** Blinn-Phong's half-vector — and it is being written as named geometric steps rather than as the one-liner, because the formula is worth *watching assemble*. `L_proj = (N·L) * N` is the part of the light vector lying along the normal: the dot product measures *how much* of `L` points along `N`, and multiplying that scalar back onto `N` rebuilds it as a vector. `L_flat = L - L_proj` is the remainder — the component lying in the surface plane, "flat" because with `N` as up it has no height; it is the shadow `L` casts on the surface. `R = L_proj - L_flat` keeps the normal-aligned part and **negates the tangential one**, and that single sign flip *is* the reflection — mirroring across a surface leaves the up part alone and reverses the sideways part. Substituting back, `L_proj - (L - L_proj) = 2*L_proj - L`, and the textbook formula falls out instead of being asserted. Nothing is on screen for 6b yet: the file is not in `CMakeLists.txt` and has no call site. The exit condition is the **vanishing highlight** — with the exponent turned up, a specular lands mid-triangle under Phong and does not exist at all under Gouraud, which evaluates lighting only at corners.
