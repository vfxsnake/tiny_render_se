...
**Step 6c is agreed and next: a Blinn-Phong shader as a standing A/B.** Same varyings, same uniforms, same diffuse — only `R·V` becomes `N·H` with `H = normalize(L + V)`. Two predictions recorded before writing it, since they are the reason to build it rather than read about it: the exponents **do not map 1:1** (the half-vector angle is roughly half the reflection angle, so matching Phong's width needs roughly 2–4× the exponent, and comparing at equal numbers would wrongly read as "Blinn is blurrier"), and the two models **disagree at grazing angles** — Phong's highlight stays circular where Blinn's stretches into an elongated streak, and the streak is the physically correct one, which is why that A/B wants the light swung nearly parallel to the surface.
...
**What is left, prioritised.** Three concepts remain that must be *seen* rather than explained: **perspective-correct interpolation** (Lesson 7 — interpolating UVs with screen-space barycentrics produces the PlayStation-1 texture swim, and it is the most valuable single item left), the **v-flip** (OBJ's UV origin is bottom-left while image rows run top-down), and **shadow acne** (Lesson 9 — a surface shadowing itself in stripes). One concept is genuinely hard and gets a full session: **tangent space** (Lesson 8), where the basis is derived from UV derivatives. Everything else compacts — the half-vector, nearest-neighbour sampling, specular maps (a texture read feeding a term that already exists), and toon shading. Two structural blockers gate Lesson 7 and are confirmed in the code rather than assumed: **`ObjLoader` has no `vt` handling at all**, so `Mesh` needs `texCoords` and `faceTexCoordIndices` on the same independent-indexing pattern as normals; and `TriangleRasterizer.cpp:203-205` divides by `w` and discards it, which is precisely the forfeit the prologue-ordering argument logged as Lesson 7's problem. Lessons 10 (ambient occlusion) and 11 (toon) are bonus in the source series and now explicitly optional. The estimate is **~7 more runs** against roughly 15 at Lesson 6's pace, and the honest reason for the gap is that **the concepts are not what costs sessions — the kick-off protocol is**; the lever is the triage in step 4, not the concept list.
