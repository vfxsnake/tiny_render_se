...
**As of 2026-09-22 — Lesson 9 (Shadow mapping) in progress: shadow map renders; lookup pass next. Lessons 0-8 complete.**
...
- **Lesson 9 - Shadow mapping: in progress — pass 1 done.** A `DepthShader` renders the mesh from a directional light (normalized `{1,1,1}`) into a second `Framebuffer`, shown on screen as greyscale depth. Measuring light-space bounds showed `lookAt` ignores eye distance (it translates by the target), so the model is fitted into the light frustum with a uniform 0.8 scale instead. Next: the shadow lookup in `MaterialShader`. Two passes over the same `Framebuffer` class: a depth-only pass from the light fills a second, CPU-only depth buffer (the shadow map), and `MaterialShader` reads it to decide whether a fragment is visible to the light. Two decisions settled the shape: the fragment reaches light space by interpolating the object-space position varying that Lesson 8 already added, so the lesson's `M⁻¹` and a 4×4 inverse are not needed; and the light stays **directional**, so `lookAt` with no `perspective()` is the orthographic light camera and shading and shadow are built from the same vector. Shadow mapping and ambient occlusion grow into `MaterialShader`, which is why it is not named after its lighting model.
...
| 9 | Shadow mapping | In progress (shadow map on screen) |
